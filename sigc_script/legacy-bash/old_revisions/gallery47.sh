...
usage(){
	...
	echo "Usage: gallery [-ChqTV] [-l | -L] [-c number_of_columns] [-e extension] [-t title] [-r relative_to] [-s source_directory] -d directory"
	...
	echo "    -d directory - Absolute link to directory with images in.  Escape spaces with a \ symbol."
	...
	echo "    -r relative directory - Directory image links should be relative to, escape spaces with a \ symbol."
	echo "    -s source directory - Use this directory as a source for images.  Images will be copied from here into the directory"
	echo "         specified using the -d option."
...
while getopts ":c:d:e:H:r:s:t:CThvlLV" flag
do
	case $flag in
		c) COLUMNS=$OPTARG;;
		C) CAPTIONS="TRUE";;
...
#check to see if source directory is specified and if so, if the directory exists.
if [[ -n $SOURCE ]]; then
	if [[ $QUIET != "TRUE" ]]; then
...
#Check gallery directory exists, if not then show usage with error message and exist with a stutus of 1
if [[ $QUIET != "TRUE" ]]; then
